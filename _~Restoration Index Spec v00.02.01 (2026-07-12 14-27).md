...
`v00.02.01 · 2026-07-12 14-27· internal working doc · status: decisions adopted; codebook v0 and engine v0 implement this spec`
...
- **Groceries:** GAO-25-107451 — downsizing contributed **~0.06pp** of the 34.5% CPI rise 2019–24, because CPI already books per-unit increases. Expected grocery Restoration Gap ≈ **0**. Published as the null test against the CPI food-at-home per-unit comparator available at print time, with the full R-CPI-SC reconciliation as an addendum when the covering vintage ships (the plan's vintage-aware two-stage convention, adopted as binding by the Phase 1 pre-registration and gate G1).
...
| 3 | Event trigger | Fires upward iff same-seller replication cost of the frozen experience rises; downward tier introduction beside an unchanged incumbent prints 0% for profiles it does not dominate and a genuine decrease for profiles it weakly dominates at lower cost (P2 refinement); symmetric negative prints mandatory | High | The two fatal-unless-fixed attacks (revealed preference; one-way ratchet); P2/P3 are the design's best defenses and cost nothing |
...
**Phase 1 — MVP (first published number).** Air travel: pre-registered route panel (top T-100 routes), paired same-flight Basic/Economy quotes, fee-schedule ledger with the verified 2025–26 event set (Southwest 5/28/25 and 4/9/26; April 2026 fee wave; AA fare-class bag surcharge 5/18/26; United carry-on standing event). Streaming sidecar: full tier ladders (near-zero cost, fully archival). Negative controls: groceries (vs the print-time CPI food-at-home per-unit comparator; R-CPI-SC addendum when its covering vintage ships) and banking (falling fees). Weights: FRED CXU bottom-two-quintile tables. Output: sector Restoration Gaps + decomposition + Exclusion Register + reconciliation table, code and line items public.
...
|---|---|---|
| Revealed preference: trading down into a still-available option is welfare-improving | fatal-unless-fixed | Trigger split (§3.1) + P2: menu expansion can never raise the index — 0% for undominated profiles, genuine decreases where a new option weakly dominates at lower cost; "welfare loss" language dropped project-wide — the claim is *replication cost*, true by construction |
...
- **v00.02.01** (2026-07-12 14-27) — Errata only; no formal statement, worked example, or decision changed (engine tests unaffected, suite green). (1) Seam-2 convention clarified: the 0.28pp/yr figure is the geometric-annualized bottom-vs-top quintile gap; the same series is ≈0.43pp/yr in simple per-year terms (cumulative 7.67pp over 2006–2023; +8.1pp through Dec 2024) — both now byte-verified against the registered R-CPI-I series, resolving the flag raised 2026-07-12 as a convention difference, not an error. (2) Grocery null comparator synced to the plan's vintage-aware two-stage convention in §4.4 and §10 (print-time CPI food-at-home per-unit comparator; R-CPI-SC addendum on vintage release) — the sync the Phase 1 adversarial review required before the freeze commit.

- **v00.02.00** (2026-07-04 21-51) — **P2 refined from "option-introduction invariance" to menu-expansion monotonicity**, surfaced by implementing the min-cost operator (`engine/restoration.py`): a new option that weakly dominates a profile's frozen experience at lower cost genuinely lowers replication cost and prints negative (consistent with P3); menu expansion can never raise RX. S2 exhibit updated with the per-profile split (carry-on 0%; personal-item −13.3%; weighted ≈ −4.0%). Propagated to §0, §2.4, §3.1, §4.1, §6 row 3, §11, the codebook (§4 and header), the G0 band's sign-test wording (logged amendment), and the site. Public brand recorded ("The Restoration Gap", plan P13). §13 open-item statuses updated (3, 5, 6, 7 closed; 1 drafted; 4, 8 partial). Worked examples are now executable: `python3 -m unittest discover engine`.
