...
`v00.03.07 · 2026-07-06 16-09 · internal working doc · companion to: Restoration Index Spec v00.02.00`
...
13. Air-travel route panel live by October. (The panel's first fully in-sample 12-month link is **October 2027**; the Q1 2027 print is the archival-denominator exhibit under the pre-registered convention in §4, plus the exact streaming link.) ~~Freeze panel pre-registration only after G0 passes.~~ **G0 passed 2026-07-04; the pre-registration is DRAFTED and committed (2026-07-06): `docs/phase1-preregistration.md` + `docs/g1-band.md` (pass criteria GA–GD, committed first per the band-before-inputs ordering) + `data/phase1-inputs.md` + `pipeline/select_routes.py` (the frozen 18-route formula, pinned by `engine/test_select_routes.py`) + `pipeline/wave-prompts.md`. Binding on owner sign-off (checkboxes in the band and pre-registration files); wave 0 dry run Sept 2026; wave 1 = 2026-10-06. Owner-action queue before wave 0: T-100 CY2024 pull + SHA-256, DB1C product-name byte-verification, CE Table 1101 detail cells, FDIC SoD pull, WS-G counsel review of the fare-search carve-out + WN manual protocol, discussant outreach (Aug).**
