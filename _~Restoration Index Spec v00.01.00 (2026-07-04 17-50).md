...
`v00.01.00 · 2026-07-04 17-50 · internal working doc · status: draft for "go" review`
...
2. **The event trigger must be split** (fatal-unless-fixed, per all three reviewers). The index fires **iff the minimum cost of replicating the base experience rises**. If Basic Economy launches at $260 beside standard Economy unchanged at $300, the index prints **exactly 0%** — that event is menu expansion, not inflation, and an index that prints positive on it measures resentment, not prices. This "option-introduction invariance" is the design's single strongest defense: it is what separates it from a junk-fee counter.
...
- **P2 — Option-introduction invariance.** Adding a product without changing existing ones leaves RX unchanged. *Basic launches at $260 beside unchanged $300 Economy ⇒ RX prints 0% — while a lowest-available-fare CPI quote prints −13.3%.* RX is not a fee counter and cannot be triggered by menu expansion.
...
**Non-events (print 0% by rule):** downward tier introduction with the incumbent's own price and inclusions unchanged; fee-label reshuffles that leave the all-in replication cost unchanged; pure price moves on an unchanged menu (those flow through P1 as ordinary price change, not as an "event").
...
| **S2 — undercut launch**: Basic $260, Economy unchanged $300 | lowest-available-fare quotes: $300→$260 = **−13.3%** (a price cut no constant-experience traveler received) | negative, share-weighted | **0.0%** (P2: option introduction) — RX is the *least* alarmist of the three here |
...
S3 is incidence weighting in action: the traveler who never carried a bag has an unchanged replication cost, and the index says so. S2 is the design's proudest moment: where a naive fee-tracker (and CPI's lowest-fare spec, in the deflationary direction) misfires, RX prints zero.
