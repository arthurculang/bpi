...
`v00.03.12 · 2026-07-10 13-29 · internal working doc · companion to: Restoration Index Spec v00.02.00`
...
| P16 | Full-basket coverage (the "you only cover 26%" answer) | Adopt the **Restoration Corridor (RX-100)** as the documented design direction (`docs/rx-100-concept.md`): a partial-identification interval over **100% of the CPI basket** — every stratum classified Measured / Tested (CPI-adequacy audit — the grocery null generalized) / Bounded (Register magnitudes), plus the point-identified B40-vs-CPI-U weighting effect as the floor. **CONCEPT only** — binding solely through a dedicated pre-registration + a new gate (G4) + discussant review; nothing touches the signed-off Phase 1. Two near-term published-data wins queued: (a) restate coverage in CPI-basket space with B40 weights (26% → ~30–35%), and (b) ship the 100%-basket weighting-effect exhibit — both folded into the August Table 1101 refresh. **Formalized into `docs/rx-100-proposal.md` (2026-07-07) for the August discussant packet** (owner decision to make it a formal proposal, 2026-07-07) | Medium — the architecture is sound and house-rule-clean (spread-only, bound language, no "true inflation"); open questions (K calibration, Tier-2 audit power, whether the first print's width leaves a publishable sign) go to the discussant | Zero-imputation on the uncovered 74% is a *disguised point estimate of zero*; the owner's objection (the remainder could move the other way) is correct, and the fix is to replace the hidden assumption with named, ordered, falsifiable bounds and publish the interval — the only design in which every point of the basket has an explicit evidentiary status, answering "what about the rest?" at every coverage level without a rival-index claim |
...
- **v00.03.12** (2026-07-10 13-29) — **Wave-0 build kit shipped + corpus audit.** Built the two sandbox tools required before wave 0: `pipeline/build_worksheet.py` (generates the monthly wave worksheet from the frozen panel + a wave date — blackout iteration, WN manual list, deterministic audit rotation) and `pipeline/validate_capture.py` (fail-loud schema + range + QC-2/3/8 checks for air/streaming/grocery records), pinned by `engine/test_wave_tools.py` (16 new tests, incl. the §2 blackout date-traces Dec-2026→Jan-5/12 and Dec-2027→Jan-11/18). Suite now 54 tests green. Pre-registration §6 Steps 0/4 updated to reference the built tools. Also ran a corpus-wide consistency audit (the workflow was interrupted mid-run; completed directly): version-stamp/amendment-log discipline held — no cross-document drift; the site Inclusion Ledger matches `data/ledger-events.json` (all 10 verified events, amounts/dates); sole open item = owner disclosure decision on surfacing the G0 pass publicly (recommend holding to the Oct pre-registration). No binding-rule or numeric change.

- **v00.03.11** (2026-07-07 15-01) — **RX-100 promoted from concept to formal proposal for external review** (`docs/rx-100-proposal.md`), per owner decision. Recasts the Restoration Corridor as an estimand (`G* = Σ w_i(ρ_i − π_i)` over the full CPI basket) + a partial-identification argument in the Manski tradition: per-stratum tier bands, the sharp aggregate interval `[L, U]`, the assumption ladder (A-K/A-R/A-Q/A-H/A-O) each with identifying content and a falsification test, the point-identified weighting floor, and the two new instruments (the generalized CPI-adequacy audit; the Improvements Register). §7 poses five pointed reviewer questions ending in "what would make you reject this," mapped to the trio's strengths (Fox/Greenlees/Jaravel). Governance: enters only via its own pre-registration + a new gate G4, after review — unchanged from the concept. Concept note `docs/rx-100-concept.md` cross-linked; `docs/discussant-shortlist.md` packet contents updated. P16 annotated. No binding-artifact or numeric change.

- **v00.03.10** (2026-07-07 06-16) — **Added decision P16 and `docs/rx-100-concept.md`: the Restoration Corridor (RX-100), a 100%-of-CPI-basket design direction** answering the owner's "you only cover 26%" objection. Core: zero-imputation on the uncovered remainder is a disguised point estimate of zero; replace it with a partial-identification interval over the whole basket — strata classified Measured / Tested (CPI-adequacy audits, the grocery null generalized from control to instrument) / Bounded (Exclusion + a new Improvements Register), with the point-identified B40-vs-CPI-U weighting effect as the floor and the interval width as a published identification-progress metric. Concept only — additive, house-rule-clean (spread-only, bound language, no "true inflation"), binding solely via a dedicated pre-registration + gate G4 + discussant review; does not touch the signed-off Phase 1 / G0 / G1 / wave schedule. Produced by a 2-of-3-lens design workflow (formal partial-identification + measurement-pragmatist converged; product/referee lenses supplied by owner review). Two near-term published-data wins queued into the August Table 1101 refresh: the coverage restatement into CPI-basket space (26% → ~30–35%) and the 100%-basket weighting-effect exhibit.

- **v00.03.09** (2026-07-06 21-56) — **Owner sign-off recorded on both Phase 1 freeze artifacts** (`docs/g1-band.md` and `docs/phase1-preregistration.md`, checkboxes checked 2026-07-06 after the plain-language decision walkthrough). The G1 pass criteria and the panel pre-registration are now fully binding under their MAY/MAY-NOT walls; collection is unblocked — wave 0 dry run September 2026, wave 1 = Tuesday 2026-10-06. §9 item 13 status updated. No numeric or rule content changed.
